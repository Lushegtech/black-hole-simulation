...
struct Engine {
    ...
    int COMPUTE_WIDTH  = 800;   // Compute resolution width (full quality)
    int COMPUTE_HEIGHT = 600;  // Compute resolution height (full quality)
    ...
    void dispatchCompute(const Camera& cam) {
        ...
        // 2) bind texture as image unit 0
        glBindImageTexture(0, texture, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA8);

        // 3) dispatch full resolution grid
